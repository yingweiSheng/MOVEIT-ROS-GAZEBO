...
def generate_launch_description():
    ...
    test_moveit_node = TimerAction(
        period = 5.0,
        actions=[
            Node(
            package="gazebo_test_commander",  # 添加包名
            executable="test_moveit",         # 可执行文件名（不是完整路径）
            parameters=params,
            output="screen",                     # 输出到屏幕
            emulate_tty=True,                    # 确保日志实时显示
        )
        ]
    )
    ld.add_action(test_moveit_node) 
    return ld
...
def my_generate_moveit_rviz_launch(ld, moveit_config):
    ...
    ld.add_action(
        DeclareLaunchArgument(
            "rviz_config",
            default_value=str(moveit_config.package_path / "config/moveit_pure.rviz"),
        )
    )
